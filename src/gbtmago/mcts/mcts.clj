...
(defn zero-mcts [{:keys [tree root state deepend transition-fn actions-fn terminal?]}]
        ...
        (if (not (terminal? state))
          (let [children (map (partial transition-fn state)
                ...
                tree (reduce (fn [t c]
                             ...
                             tree
                             children)
                tree (update tree state assoc :children children :expanded true)
                tree (loop [t tree
                            n state
                            new-t
                            (-> t (update n (fn [o]
                       ...
                       (if (not= n root)
                         ...
                         new-t))]
            tree)
          (let [deepend-output (deepend "predict" {:state (:board state)})
                v (last deepend-output)
                tree (update tree state assoc :expanded true)
                tree (loop [t tree
                            n state
                            new-t
                            (-> t (update n (fn [o]
